...
import cv2
from environmentPlaza import Environment
import rospy, tf
...
class NeuralNetwork(nn.Module):
	def __init__(self):
		...
		self.number_of_iterations = 300#10000
...
def train(model, start):
	...
	optimizer = optim.Adam(model.parameters(), lr=0.01)#0.0025)
	...
	while iteration < model.number_of_iterations:
		...
		epsilon*=(0.99**(0.05*(  ((iteration+1)/((taken[iteration+1]+1)/(taken[0]+1))) /fails)))
