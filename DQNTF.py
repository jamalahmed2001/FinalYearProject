...
def play_game(env, TrainNet, TargetNet, epsilon, copy_step):
    ...
    while not done:
        ...
        observations, reward, done= env.getReward(action)
        observations = formatImg(observations)
        rewards += reward
        if done:
            reward = -200
            env.reset()
...
def formatImg(image):
    ...
    cv2.imshow("Camera view diff", image_data)#
    cv2.waitKey(3)
    # image_data[image_data > 0] = 255
    image = np.reshape(image_data, (84, 84, 1))
...
def main():
    ...
    log_dir = '/home/jamalahmed2001/catkin_ws/src/simulated_homing/src/logs/dqn/' + current_time
